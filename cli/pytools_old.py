...
from lztools.pytools import pip_package
...
def create(name, parent):
    ...
    pip_package.create(name, pack=parent)
...
@click.option("-d", is_flag=True, default=False, help="install in develop mode")
def install(paths, upload, a, add, d):
    ...
    if not paths:
        pytools.local_install(".", should_upload, add, upload, d)
    else:
        for x in paths:
            pytools.local_install(str(Path(x).absolute()), should_upload, add, upload, d)
